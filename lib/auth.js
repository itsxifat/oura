    ...
    async signIn({ user, account }) {
      if (account.provider === 'google') {
        ...
        if (!dbUser) {
          ...
          // The guest placed orders with this email before signing in.
          // Promote them to a full Google-authenticated user while keeping
          // all their orders, known IPs, devices, and other data intact.
          await User.findByIdAndUpdate(dbUser._id, {
            provider:   'google',
            image:      user.image || dbUser.image,
            isVerified: true,
            // Keep their name if they already had one set
            ...((!dbUser.name || dbUser.name === 'Guest') && user.name ? { name: user.name } : {}),
          });
          dbUser = await User.findById(dbUser._id); // re-fetch updated doc
        }
