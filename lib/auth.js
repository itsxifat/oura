...
export const authOptions = {
  ...
  callbacks: {
    async signIn({ user, account }) {
      ...
      // Track IP for the signing-in user.
      // connectDB() may not have been called for credentials logins (only called
      // inside the Google branch above), so ensure it's established.
      // trackUserActivity() also calls connectDB() internally now, so this is a safeguard.
      if (user.id) {
        await connectDB();
