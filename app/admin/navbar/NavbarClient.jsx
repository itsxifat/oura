    ...
    <div className="min-h-screen bg-[#faf9f6] font-manrope p-4 md:p-8 pt-24 lg:pt-8 text-gray-900" ref={containerRef}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-8 border-b border-gray-200 pb-8 gap-6">
          <div>
            <span className="text-[#D4AF37] font-bold uppercase tracking-[0.3em] text-xs">Site Layout</span>
          ...
          <button 
            onClick={handleSave} 
            disabled={isSaving}
            className="w-full md:w-auto bg-black text-white px-8 py-3 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-[#D4AF37] transition-all flex items-center justify-center gap-2 disabled:opacity-70 shadow-lg hover:shadow-xl hover:-translate-y-0.5"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
             ...
             <div className="anim-entry bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col h-[500px]">
                <div className="mb-4 pb-4 border-b border-gray-100">
                   <h3 className="font-bold text-xs uppercase tracking-widest mb-1 flex items-center gap-2">
                      <Layers size={14} className="text-[#D4AF37]"/> Available Categories
                   </h3>
                   <div className="relative mt-3">
                      ...
                      <input 
                        ...
                        className="w-full pl-9 p-2 bg-gray-50 rounded-lg text-xs outline-none focus:bg-white focus:border-[#D4AF37] border border-transparent transition-all"
                      />
                   </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-gray-200">
                   {filteredCategories.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-gray-300 text-center">
                         ...
                         <p className="text-[10px] uppercase">No matches found</p>
                      </div>
                   ) : (
                      filteredCategories.map(cat => (
                        ...
                        >
                          ...
                          <Plus size={14} className="text-gray-300 group-hover:text-[#D4AF37] opacity-0 group-hover:opacity-100 transition-all flex-shrink-0" />
                        </button>
                      ))
                   )}
                </div>
          ...
          <div className="lg:col-span-8">
             <div className="anim-entry bg-white rounded-2xl shadow-sm border border-gray-200 min-h-[500px] flex flex-col">
                {/* Editor Header */}
                <div className="px-8 py-6 border-b border-gray-100 bg-[#faf9f6]/50">
                   <div className="flex items-center gap-3">
                      <div className="p-2 bg-black text-white rounded-lg"><Layout size={18}/></div>
                      <div>
                         <h2 className="font-bodoni text-xl text-black">Active Menu Structure</h2>
                         <p className="text-[10px] text-gray-400 uppercase tracking-widest mt-0.5">Live Preview</p>
                      </div>
                   </div>
                </div>
                ...
                <div className="p-8 flex-1">
                   {navbarLinks.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-gray-300 py-20 border-2 border-dashed border-gray-100 rounded-xl">
                         ...
                         <AnimatePresence>
                            {navbarLinks.map((link, i) => (
                               <motion.div 
                                 ...
                                 layout
                                 className="border border-gray-200 rounded-xl overflow-hidden hover:shadow-md hover:border-[#D4AF37]/30 transition-all bg-white group"
                               >
                                  {/* Parent Item */}
                                  <div className="p-4 flex items-center justify-between bg-white z-10 relative">
                                     <div className="flex items-center gap-4">
                                        <span className="w-6 h-6 rounded bg-gray-100 text-[10px] font-bold text-gray-500 flex items-center justify-center border border-gray-200 font-mono">
                                           {String(i + 1).padStart(2, '0')}
                                        </span>
                                        <div>
                                           <span className="font-bodoni font-bold text-lg text-gray-900 block">{link.label}</span>
                                           <span className="text-[10px] text-gray-400 font-mono bg-gray-50 px-1.5 py-0.5 rounded tracking-wide">{link.href}</span>
                                        </div>
                                     </div>
                                     <button 
                                  ...
                                  {link.children && link.children.length > 0 && (
                                     <div className="bg-[#faf9f6]/50 p-4 pt-0 border-t border-gray-50">
                                        <div className="mt-4 pl-3 space-y-3 relative">
                                           ...
                                           {link.children.map((child, j) => (
                                              <div key={j} className="relative pl-8">
                                                 ...
                                                 {/* Child Card */}
                                                 <div className="bg-white p-3 rounded-lg border border-gray-100 shadow-sm flex flex-col gap-2 relative">
                                                    ...
                                                    {child.children && child.children.length > 0 && (
                                                       <div className="flex flex-wrap gap-1.5 mt-1">
                                                          {child.children.map((grand, k) => (
                                                             <span key={k} className="text-[9px] font-bold text-gray-500 bg-gray-50 border border-gray-100 px-2 py-0.5 rounded-full flex items-center gap-1.5">
